...
from dino_runner.utils.constants import CLOUD, BG, ICON, SCREEN_HEIGHT, SCREEN_WIDTH, TITLE, FPS, DEFAULT_TYPE, GAMEOVER
...
class Game:
    ...
    def draw(self):
        ...
        self.draw_cloud()
        self.draw_cloud1()
        if self.score.count > 200 and self.score.count < 400:
            self.screen.fill((0, 0 ,0))
            self.draw_background()
            self.draw_cloud()
            self.draw_cloud1()
        elif self.score.count > 500:
            self.screen.fill((199, 66 ,37))
            self.draw_background()
            self.draw_cloud()
            self.draw_cloud1()
            self.menu.draw(self.screen, 'Demon Mode', 1000, 100)    
